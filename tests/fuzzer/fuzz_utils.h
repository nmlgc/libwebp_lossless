...
static inline auto ArbitraryWebPConfig() {
  return fuzztest::Map(
      [](int lossless, int quality, int method, int image_hint, int segments,
        ...
        config.lossy_03 = segments;
        config.lossy_04 = sns_strength;
        config.lossy_05 = filter_strength;
        config.lossy_06 = filter_sharpness;
        config.lossy_07 = filter_type;
        config.lossy_08 = autofilter;
        config.lossy_09 = alpha_compression;
        config.lossy_10 = alpha_filtering;
        config.lossy_11 = alpha_quality;
        config.lossy_12 = pass;
        config.lossy_13 = 1;
        config.lossy_14 = preprocessing;
        config.lossy_15 = partitions;
        config.lossy_16 = 10 * partition_limit;
        config.lossy_17 = emulate_jpeg_size;
        config.thread_level = thread_level;
        config.lossy_18 = low_memory;
        config.near_lossless = 20 * near_lossless;
        config.exact = exact;
        config.use_delta_palette = use_delta_palette;
        config.lossy_19 = use_sharp_yuv;
